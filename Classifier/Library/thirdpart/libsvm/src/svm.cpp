...
public:
	Solver(const svm_parameter *params) {
		printf_output = (FILE *)params->printf_output;
	};
	virtual ~Solver() {};
...
static void sigmoid_train(
	...
	double& A, double& B, const svm_parameter *param)
{
...
static void multiclass_probability(int k, double **r, double *p, FILE * printf_output)
{
	int t,j;
	...
	double Cp, double Cn, double& probA, double& probB)
{
	...
	sigmoid_train(prob->l,dec_values,prob->y,probA,probB,param);
	free(dec_values);
	free(perm);
}
	...
	const svm_model *model, const svm_node *x, double *prob_estimates)
{
	    ...
	    model->probA!=NULL && model->probB!=NULL)
	{
		...
		multiclass_probability(nr_class,pairwise_prob,prob_estimates, (FILE *)model->param.printf_output);
	...
	for(int i=0;i<l;i++)
	{
		...
		if(param.kernel_type == PRECOMPUTED)
			...
			for (int j = 0; j < p->dim; j++)
				if (p->values[j] != 0.0)
					fprintf(fp,"%d:%.8g ",j+1, p->values[j]); // @ DAG 2013/05/27: Fix dense mode 1-based indexing
#else
		const svm_node *p = SV[i];
...
static char* readline(FILE *input, std::vector<char> line)
{
	...
	if(fgets(&line[0], (int)line.size(), input) == NULL)
		return NULL;

	while(strrchr(&line[0],'\n') == NULL)
	{
		line.resize(line.size() * 2);
		//max_line_len *= 2;
		//line = (char *) realloc(line,max_line_len);
		len = (int) strlen(&line[0]);
		if(fgets(&line[len], (int)line.size()-len,input) == NULL)
			break;
	}
	return &line[0];
}
...
bool read_model_header(FILE *fp, svm_model* model)
{
	...
	while(1)
	{
		...
		if(strcmp(cmd,"svm_type")==0)
		{
			...
			{
				if (param.printf_output)
					fprintf((FILE *)param.printf_output, "Unknown SVM type.\n");
				return false;
			}
		}
		...
		{		
			...
			{
				if (param.printf_output)
					fprintf((FILE *)param.printf_output, "Unknown kernel function.\n");
				return false;
			}
		}
