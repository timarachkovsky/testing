% HISTORY_TOOTHEDBELT_DEFECT_SECOND_SHEAVE function returns status of smooth belt
% defect of history
% 
% Defect requirements:
%     main:
...
function [similarityHistory, historyDangerous] = history_toothedBelt_DEFECT_SECOND_SHEAVE(defectStruct, myFiles)

    sheaveFreq2Tag = 30; % driving sheave frequency tag
    modTag = [31 30]; % [meshingFreq +(-) sheaveFreq1] tag
    
    % ACCELERATION ENVELOPE SPECTRUM evaluation
    [statusEnvAcc, dangerEnv] = accEnvSpectrumEvaluation(defectStruct.accelerationEnvelopeSpectrum, sheaveFreq2Tag, modTag, myFiles);
    
    % ACCELERATION SPECTRUM evaluation
    [statusAcc, dangerAcc] = accSpectrumEvaluation(defectStruct.accelerationSpectrum, sheaveFreq2Tag, modTag, myFiles);
...
% spectrum 
function [weightsStatus, dangerStatus] = accEnvSpectrumEvaluation(domain, sheaveFreq2Tag, modTag, myFiles)
...
function [weightsStatus, dangerStatus] = accSpectrumEvaluation(domain, sheaveFreq2Tag, modTag, myFiles)
