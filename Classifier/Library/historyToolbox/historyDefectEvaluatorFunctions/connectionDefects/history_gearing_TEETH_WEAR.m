...
function [similarityHistory, historyDangerous] = history_gearing_TEETH_WEAR(defectStruct, myFiles)
...
function [status, danger] = evaluateGearMeshFrequency(domain, shaftTag, modWhithShaft, myFiles)
    ...
    if ~isempty(shaftPositions)
        ...
        if nnz(newStructMod.status)
            if nnz(newStructMod.position == 1)
                if nnz(newStructMod.magnitudes(newStructMod.position == 1) >= newStructMod.magnitudes) == ...
        ...
        status = (sum(bsxfun(@times, weights, estimationsMod))*firstLargest) + sum(statusShaft);
