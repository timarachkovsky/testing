% HISTORY_SMOOTHBELT_DEFECT_FIRST_SHEAVE function returns status of smooth belt
% defect of history
% 
% Defect requirements:
%     main:
...
function [similarityHistory, historyDangerous] = history_smoothBelt_DEFECT_FIRST_SHEAVE(defectStruct, ~)

    sheaveFreq1Tag = 29; % driving sheave frequency tag
    
    % ACCELERATION ENVELOPE SPECTRUM evaluation
    [statusEnvAcc, dangerEnv] = accEnvSpectrumEvaluation(defectStruct.accelerationEnvelopeSpectrum, sheaveFreq1Tag);
    
    % ACCELERATION SPECTRUM evaluation
    [statusAcc, dangerAcc] = accSpectrumEvaluation(defectStruct.accelerationSpectrum, sheaveFreq1Tag);
...
% spectrum 
function [weightsStatus, dangerStatus] = accEnvSpectrumEvaluation(domain, sheaveFreq1Tag)
...
function [weightsStatus, dangerStatus] = accSpectrumEvaluation(domain, sheaveFreq1Tag)
