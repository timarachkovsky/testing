% HISTORY_GEARING_GROOVES_CHIPPED_TEETH
%
% Developer:              Kosmach N.
...
%   
function [similarityHistory, historyDangerous] = history_gearing_GROOVES_CHIPPED_TEETH(defectStruct, myFiles)
...
function [status, danger] = evaluateGearMeshFrequency(domain, shaftTag, modWhithShaft, myFiles)
    ...
    if ~isempty(shaftPositions)
        ...
        if nnz(newStructMod.status)
            if nnz(newStructMod.position == 3)
                if nnz(newStructMod.magnitudes(newStructMod.position == 3) >= newStructMod.magnitudes) == ...
        ...
        status = (sum(bsxfun(@times, weights, estimationsMod))*thirdLargest) + sum(statusShaft);
    else
        status = 0;
        statuModDanger = 0;
    end
