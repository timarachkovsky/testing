classdef interferenceFrequencyEstimator < frequencyEstimator
    ...
    methods (Access = public)
        ...
        % Constructor method
        function myFrequencyEstimator = interferenceFrequencyEstimator(File, config)
            if nargin < 2
                config = [];
            end 
            ...
            printStage(myFrequencyEstimator, 'The interference frequency estimator was created')
        end
        ...
        function [result,myFrequencyEstimator] = getFrequencyEstimationWithAccuracy(myFrequencyEstimator, accuracy)
            ...
            myBaseFrequencies = myFrequencyEstimator.baseFrequencies;
            frequenciesNumber = length(myBaseFrequencies);  %Shaft frequencies gotten from kinematics and the main shaft's speed collection.
            printStage(myFrequencyEstimator, ['Start estimation acceleration envelope spectrum interference method with ' accuracy ' accuracy.'])
            ...
            [result] = makeDecision(myFrequencyEstimator, interferenceResults, accuracy);
...
%                 myFrequencyEstimator.accurateFrequency = result.frequency;
    ...
    methods (Access = private)
        ...
        function [result] = createInterferenceResults(myFrequencyEstimator, accuracy, baseFrequency, shaftSchemeName)
            ...
            str = '';  %File name addition to sign validation frames.
            mainFramesIndexes = 1:mainFramesNumber;
            IdxVal = 1:mainFramesNumber;
            ...
            if myConfig.Attributes.fullSavingEnable
                close all
                %test plotting
                plotFrameResults(myResults, myNominalFrequency, [str 'AccelerEnvel_'], '', peaksConf)
            end
            
            % Form interference vector of spectum frames to find peaks with
            % great prominence.
            if nnz(IdxVal)
            ...
            if additionalFramesNumber > 0 && nnz(~isnan(frequencyValueMain))>0
                ...
                %test plotting
                if myConfig.Attributes.fullSavingEnable
                    plotFrameResults(myResults, myNominalFrequency, [str 'AccelerEnvel_Addit_'], '', peaksConf);
                end
            ...
            if plotEnable == 1
                ...
                figure('Name', 'Interference method of frequency estimation', 'NumberTitle', 'off', ...
                ...
                for i = 1:1:subplotsNumber
                    ...
                    title(framesCaptions{i});   %['Normolized ',num2str(i),' frame']
                end
...
%                     PicName = sprintf('%10.5f_AccelerEnvel_%s.jpg', myNominalFrequency, 'allFramesAndInterferenceResults');
