function [psi, xval] = swd_morl_mexh(scale, Fs, formFactor)
...
lb = -0.06*scale;   % lowwer bounds
ub = 0.06*scale;    % upper bound
...
psi(middlePos+1:end) = (4/pi^2)*(pi^2/4 - wt2.^2).*exp(-2*wt2.^2/pi^2); % @mexh_morl left part
psi(1:middlePos) = cos(wt1).*exp(xval(1:middlePos)/k);     % @mexh_morl right part
