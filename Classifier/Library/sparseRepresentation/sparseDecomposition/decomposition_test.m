...
function [restored,sparsePeaks,sparseModes,basis, measurementResult] = decomposition_test(file, config)
...
%% ____________________ Default Parameters ____________________________ %%
   ...
   config = fill_struct(config, 'stopMethod', 'maxIterationsNumber');  % maxIterationsNumber \ maxRmsPart
   config = fill_struct(config, 'maxIterationsNumber', '10000');   %Max decomposition
                                                                % iterations number
   config = fill_struct(config, 'maxRmsPart', '0.98');  % Max rms part of the SWD component
   ...
   config = fill_struct(config, 'deadzone', '0'); % Zone around the decompositin
	...
	if ischar(config.maxRmsPart)
        config.maxRmsPart = str2num(config.maxRmsPart);
    end
    ...
    if config.measurementEnable == 1
		switch(config.stopMethod)
			...
			case 'maxRmsPart'
				rmsPartVector = linspace(0.25,config.maxRmsPart,config.measurementPointsNumber);
				iterationsVector = zeros(size(rmsPartVector));
                index = 0;
				signalRms = rms(file.signal);
				validVector = zeros(size(rmsPartVector));
                currentValidVector = zeros(size(rmsPartVector));
                rmsVector = zeros(size(iterationsVector));
...
%% ------------------------- Calculation ------------------------------- %%
        ...
        if config.measurementEnable == 1
			switch(config.stopMethod)
				...
				case 'maxRmsPart'
					...
					swdSignal = restored(padsize+1:end-padsize)';
					if rms(swdSignal)/signalRms <= config.maxRmsPart || index < config.measurementPointsNumber


                       currentValidVector = (rmsPartVector<=rms(swdSignal)/signalRms);
                       positions = currentValidVector - validVector;
                       index = index + nnz(positions);

                        if index > config.measurementPointsNumber
                            if isempty(measurementResult)
                                measurementResult.table(1,:) = swdMetrics(file,swdSignal);
                            else
                                measurementResult.table(end+1,:) = swdMetrics(file,swdSignal);
                            ...
                            if iterationsVector(end) == 0
                                iterationsVector(end) = ii;
                            end
                            iterationsVector = unique(iterationsVector(iterationsVector>0));
                            if rmsVector(end) == 0
                                rmsVector(end) = config.maxRmsPart;
                            end
                            rmsVector = unique(rmsVector(rmsVector>0));
                            measurementResult.table = measurementResult.table(1:length(iterationsVector),:);
                            break
                        end
                       
                        if nnz(positions)>1
                            iterationsVector(find(positions)) = ones(nnz(positions),1)*ii;
                            rmsVector(find(positions)) = ones(nnz(positions),1)*rmsPartVector(index);
                           ...
                           if isempty(measurementResult)
                            ...
                            if index && ~validVector(index) 
                                iterationsVector(index) = ii;
                                rmsVector(index) = rmsPartVector(index);
                        ...
                        validVector = currentValidVector;
                        
                    elseif rms(swdSignal)/signalRms > config.maxRmsPart
                        currentValidVector = (rmsPartVector<=rms(swdSignal)/signalRms);
                        positions = currentValidVector - validVector;
    ...
    if config.measurementEnable == 1
        ...
        if(strcmp(config.stopMethod, 'maxRmsPart'))
            measurementResult.rmsPartVector = rmsVector;
        end
...
%% _______________________ Plot Results _______________________________ %%
    if config.plotEnable
        ...
        figure('Visible',config.plotVisible);
%         subplot(2,1,1);
%         plot(file.basis);
%         xlabel('Samples');
%         ylabel('Magnitude');
...
%         subplot(2,1,2);
        ...
        title( ['Sparse Wavelet Decomposition. Iterations Number = ',num2str(config.maxIterationsNumber) ]);
        set(gcf,'color','w');
